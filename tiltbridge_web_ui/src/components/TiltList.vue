  ...
  <div>
    ...
    <main>
      ...
      <div class="bg-white px-2 py-5 sm:px-6">
        <!-- Scrolls the table rather than the page when a row still cannot fit. max-w-full so
             it shrinks to the phone first: a table inside an overflow-x box otherwise takes
             its max-content width and drops the temperature column off the right edge. -->
        <div class="overflow-x-auto">
          <table class="min-w-full max-w-full divide-y divide-gray-200">
            ...
            <tr>
              ...
              <th scope="col" class="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">{{ $t('tilt_list.rssi_column') }}</th>
            ...
            <tr v-for="(tilt, tiltIdx) in tiltStore.tilts" :key="tilt.rowKey" :class="tiltIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'">
              <td class="relative px-3 sm:px-6 py-4 text-sm font-medium text-gray-900">
              ...
              <td class="px-3 sm:px-6 py-4 whitespace-nowrap text-sm font-medium hidden sm:table-cell" :title="rssiTooltip(tilt)">
